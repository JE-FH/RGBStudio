...
KeyTriggerFactory::KeyTriggerFactory()
{
	...
	spec.add_field("trigger_on_press", std::make_unique<ConfigBoolTypeDesc>(), true);
	spec.add_field("trigger_on_release", std::make_unique<ConfigBoolTypeDesc>(), true);
	spec.add_field("trigger_on_repeat", std::make_unique<ConfigBoolTypeDesc>(), true);
}
...
const DynamicConfigSpec& KeyTriggerFactory::get_config_spec()
{
	return spec;
}