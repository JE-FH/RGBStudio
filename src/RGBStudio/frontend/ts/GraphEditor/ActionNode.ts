...
export class ActionNode extends GraphNode {
	...
	constructor(container: WidgetContainer, name: string, dragable: boolean, graphConnectorService: IGraphConnectorService) {
		...
		this._source = new Connector("source", this, ConnectorDirection.Source, ConnectorType.Action, graphConnectorService, false, false);
		this._target = new Connector("target", this, ConnectorDirection.Target, ConnectorType.Trigger, graphConnectorService, false, false);
	...
	AddLineEndToTarget(lineEnd: LineEnd) {
		this._target.AddLineEnd(lineEnd);
	}

	AddLineEndToSource(lineEnd: LineEnd) {
		this._source.AddLineEnd(lineEnd);
    }
}
